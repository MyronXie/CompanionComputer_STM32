...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	...
	if(htim->Instance == TIM7)		// TIM7: Read & Send Battery Message (50Hz)
	{
		Battery_Management();
	}
}
...
void Mavlink_Decode(mavlink_message_t* msg)
{
	switch(msg->msgid)
	{
		...
		case MAVLINK_MSG_ID_COMMAND_ACK:
			...
			printf("\r\n [FMU]  #77 : %4d,%d", mavCmdAck.command, mavCmdAck.result);	// Param progess is dummy
			break;
		...
		switch(battCycleCnt)
		{
			...
			case 0x29:
				if(battAutoOff)
				{
					...
					if((battAutoOff+1)%2)
					{
						printf("\r\n [ACT]  Auto-Off Attempt#%d",(battAutoOff+1)/2);
